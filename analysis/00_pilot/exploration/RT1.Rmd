---
title: "Connectives + QUD Study: RT 1"
author: Morgan Moyer
date: July 12, 2022
...
ggplot(agr,aes(x=Conj, y=meanRT, fill=QUDType)) +
  ...
  ggtitle("mean RT for first chunk")
```
...
ggplot(agr,aes(x=Number, y=meanRT, fill=QUDType)) +
  ...
  # scale_fill_manual(values=cbPalette) +
  # scale_color_manual(values=cbPalette) +
  ggtitle("mean RT for first chunk, 7/8, 1/2")
```
...
agr <- d %>%
  ...
  summarize(meanRT_chunk1 = mean(chunk1), CILow = ci.low(chunk1), CIHigh = ci.high(chunk1)) %>%
...
dodge <- position_dodge(.9)
ggplot(agr,aes(x=Letters, y=meanRT_chunk1,alpha=QUDType, fill=Letters)) +
  ...
  # scale_fill_manual(values=cbPalette) +
  # scale_color_manual(values=cbPalette) +
  ggtitle("mean RT for first chunk, individual trials in order 1, 2, 7, 8")
```
...
# reoder the levels so that we can see side-by-side the comparisons
# df_chunk1$Letters <- factor(df_chunk1$Letters, levels=c("EBT","LHN","TYK","IYG","JLP","XDS","QDR","UOP"))

dodge <- position_dodge(.9)
ggplot(agr,aes(x=QUDType, y=meanRT_chunk1, fill=QUDType)) +
  facet_wrap(~Number, ncol=2) +
  # facet_wrap(~Wh, ncol=2) +
  geom_bar(position=dodge,stat="identity") +
  geom_errorbar(aes(ymin=YMin,ymax=YMax),width=.25,position=dodge) +
  # scale_fill_manual(values=cbPalette) +
  # scale_color_manual(values=cbPalette) +
  ggtitle("mean RT for first chunk, individual trials in order 3-6, 9-12")
```
...
```{r, fig.width=10,fig.height=5}
dodge <- position_dodge(.9)
ggplot(agr,aes(x=QUDType, y=meanRT_chunk1, fill=Letters)) +
  # facet_wrap(~Letters, ncol=2) +
  # facet_wrap(~Wh, ncol=2) +
  geom_bar(position=dodge,stat="identity") +
  geom_errorbar(aes(ymin=YMin,ymax=YMax),width=.25,position=dodge) +
...
ggplot(agr,aes(x=Conj, y=meanLogRT, fill=QUDType)) +
  ...
  # scale_fill_manual(values=cbPalette) +
  # scale_color_manual(values=cbPalette) +
  ggtitle("mean logRT for first chunk")
```
...
agr <- d %>%
  ...
  # is.numeric(chunk1) %>%
  group_by(Letters,Conj,QUDType) %>% #AnswerQUDRelevant,Mention,QUDType
  summarize(meanLogRT = mean(log(chunk1))) 
  # mutate(YMin = meanLogRT - CILow, YMax = meanLogRT + CIHigh)
