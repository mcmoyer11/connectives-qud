...
ggplot(agr,aes(x=Number, y=meanRT, fill=QUDType)) +
  ...
  # scale_fill_manual(values=cbPalette) +
  # scale_color_manual(values=cbPalette) +
  ggtitle("mean RT for second chunk, 7/8, 1/2")
```
...
dodge <- position_dodge(.9)
ggplot(agr,aes(x=Letters, y=meanRT_chunk2,alpha=QUDType, fill=Letters)) +
  ...
  # scale_fill_manual(values=cbPalette) +
  # scale_color_manual(values=cbPalette) +
  ggtitle("mean RT for second chunk, individual trials in order 1, 2, 7, 8")
```
...
# reoder the levels so that we can see side-by-side the comparisons
# df_chunk2$Letters <- factor(df_chunk2$Letters, levels=c("EBT","LHN","TYK","IYG","JLP","XDS","QDR","UOP"))
...
```{r, fig.width=10,fig.height=5}
dodge <- position_dodge(.9)
ggplot(agr,aes(x=QUDType, y=meanRT_chunk2, fill=Letters)) +
  # facet_wrap(~Letters, ncol=2) +
  # facet_wrap(~Wh, ncol=2) +
  geom_bar(position=dodge,stat="identity") +
  geom_errorbar(aes(ymin=YMin,ymax=YMax),width=.25,position=dodge) +
...
agr <- d %>%
  ...
  # is.numeric(chunk2) %>%
  group_by(QUDType,Conj,AnswerQUDRelevant,Mention) %>%
  summarize(meanLogRT = mean(log(chunk2)), CILow = ci.low(log(chunk2)), CIHigh = ci.high(log(chunk2))) %>%
  mutate(YMin = meanLogRT - CILow, YMax = meanLogRT + CIHigh)
...
ggplot(agr,aes(x=Conj, y=meanLogRT, fill=QUDType)) +
  ...
  # scale_fill_manual(values=cbPalette) +
  # scale_color_manual(values=cbPalette) +
  ggtitle("mean logRT for second chunk")
```
...
agr <- d %>%
  ...
  # is.numeric(chunk2) %>%
  group_by(Letters,Conj,QUDType) %>% #AnswerQUDRelevant,Mention,QUDType
  summarize(meanLogRT = mean(log(chunk2))) 
  # mutate(YMin = meanLogRT - CILow, YMax = meanLogRT + CIHigh)
